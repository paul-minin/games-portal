...
if(-not (Test-Path .git)){
  git init
  git add .
  git commit -m "Initial commit — Standort" -q
  git branch -M main
}
...
if($RemoteUrl){
  ...
  Write-Host "Enable GitHub Pages in repo Settings: Branch 'main', folder '/'." -ForegroundColor Cyan
  exit
}
